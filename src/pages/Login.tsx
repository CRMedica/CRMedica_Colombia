...
export default function Login({ onLogin }: LoginProps) {
  ...
  // ── Estado login (original, sin cambios) ──
  ...
  // ── REGISTRO (usa /api/auth/register que ya existe en server.ts) ──
  ...
  // ── RECUPERAR CONTRASEÑA (requiere endpoint en server.ts) ──
  const handleForgot = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForgotSent(true); // Siempre mostrar éxito (no revelar si el correo existe)
    } catch {
      setForgotSent(true);
    } finally {
      setLoading(false);
              ...
              {mode === "login" && (
                  ...
                  >
                    ...
                    <div className="text-center space-y-4">
                      {/* Antes redirigía a "/" — ahora abre el formulario correcto */}
                      <button
                          type="button"
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <button
                            ...
                            className="flex items-center justify-center gap-2 py-3 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors text-sm font-medium"
                        >
